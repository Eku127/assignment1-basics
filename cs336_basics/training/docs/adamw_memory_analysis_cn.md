...
激活值（Activations）是神经网络在前向传播过程中计算出的**中间结果张量**，这些值需要被保存在内存中，以便在反向传播时计算梯度。具体来说，激活值包括：

1. **每一层的输入和输出**：如 Transformer Block 的输入、注意力层的输出、FFN 层的输出等
2. **需要参与梯度计算的中间值**：如归一化层的输入（用于计算归一化统计量）、注意力分数（如果使用了 dropout 或需要计算注意力权重梯度）
3. **残差连接的输入**：因为残差连接需要原始输入来计算梯度
4. **非线性激活函数的输入**：如 SiLU/Swish 激活函数的输入，用于计算激活函数的梯度

**需要计算哪些激活值？**

在 Transformer 中，我们需要存储以下激活值：
- 每个 Transformer Block 的输入（用于残差连接）
- 注意力层和 FFN 层的所有中间输出
- 归一化层的输入（用于计算归一化统计量）
- 所有需要参与反向传播计算的中间张量

注意：**不需要存储**那些可以通过其他已存储值重新计算出来的中间结果，或者不参与梯度计算的临时变量。


所以本质上来说，激活值就是计算每一个可训练的参数矩阵运算之后的结果的shape的大小，因为需要记住这些才能后续计算前面可训练参数的梯度
...
- 注意力分数 (QK^T):
  ...
  **为什么是这个形状？**
  ...
  在多头注意力中，经过 reshape 后：
  - Q 的形状: `(B, h, L, d_k)`，其中 `d_k = d_model / h`
  - K 的形状: `(B, h, L, d_k)`
  
  计算注意力分数时，对每个头分别计算 `QK^T`：
  - 对于每个头，Q 是 `(L, d_k)`，K 是 `(L, d_k)`
  - `QK^T` 的结果是 `(L, L)`，表示序列中每个位置对每个位置的注意力分数
  - 矩阵 `(L, L)` 中，`[i, j]` 位置表示第 `i` 个位置对第 `j` 个位置的注意力分数
  - 因为有 `h` 个头，所以是 `(h, L, L)`
  - 因为有 `B` 个批次，所以最终形状是 `(B, h, L, L)`
  
  **注意**：虽然公式写的是 `Q^T K`，但实际实现中通常计算 `QK^T`（即 `Q @ K.transpose(-2, -1)`），这样可以直接得到 `(L, L)` 的注意力分数矩阵。
- Softmax输出（需保存用于反向传播）:
  ```
  B × h × L × L
  ```
